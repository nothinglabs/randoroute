...
await context.addInitScript(() => localStorage.setItem('jra-map-state-1', 'oregon-opus-1'));
const page = await context.newPage();
const errors = [];
page.on('pageerror', (error) => errors.push(error.message));
await page.goto(site.url, { waitUntil: 'load' });
await page.waitForFunction(() => window.Region?.id === 'oregon-opus-1' && window.map,
  { timeout: 30000 });
...
const oregonArchiveRequests = site.requests.filter((request) =>
  /\/maps\/oregon-opus-1\/(?:basemap|roads|overlays)\.pmtiles/.test(request.url));

check('the real app opens in Oregon',
  observed.region === 'oregon-opus-1' && observed.dataRoot === 'maps/oregon-opus-1',
...
check('the live style points at Oregon\'s real basemap and road archives',
  sourceText.includes('maps/oregon-opus-1/basemap.pmtiles')
    && sourceText.includes('maps/oregon-opus-1/roads.pmtiles'), sourceText);
