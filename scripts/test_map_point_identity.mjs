...
// Three lines, in a fixed order: the segment, the place, and the region --
// answered from the same baked place index the search box uses, so it works
// offline and costs no request. The region is what makes a bare "Point on map"
// useful: without it the card can say nothing at all about where in the state
// the tap landed.
...
check('a segment leads with its own name, then the region',
  segmentCard.lines[0] === 'Interurban Avenue South'
    && segmentCard.lines.includes('Tukwila'), JSON.stringify(segmentCard));
check('in a fixed order: segment, place, region',
  segmentCard.order[0] === 'readout-identity-segment'
