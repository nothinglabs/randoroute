...
assert.match(html, /id="settingsHelpDialog"[\s\S]*?help-scroll-hint">Scroll ↓[\s\S]*?id="presetInfoDialog"[\s\S]*?help-scroll-hint">Scroll ↓[\s\S]*?id="layersHelpDialog"[\s\S]*?help-scroll-hint">Scroll ↓[\s\S]*?id="routeTipsDialog"[\s\S]*?help-scroll-hint">Scroll ↓/,
  'long help and preset-information screens should make their scroll affordance visible');
...
assert.match(html, /<h3>Presets<\/h3>[\s\S]*?The Randonneur[\s\S]*?Weekend Wanderer[\s\S]*?Casual Cruiser/,
...
assert.match(routeHelp, /<h3>Choose Route<\/h3>[\s\S]*?Tap A–E[\s\S]*?Click <b>Details<\/b>[\s\S]*?<h3>Details<\/h3>[\s\S]*?Concerns:[\s\S]*?All Steps:[\s\S]*?Google Street View[\s\S]*?Safety ratings &amp; map colors[\s\S]*?Dashed light blue — Designated bike route/,
