...
function bikeUse(seg) {
  const f = seg.flags || 0;
  if (f & FLAG_DESIGNATED) return 'Designated bike route or trail';
  if (f & FLAG_INFRA) return 'Dedicated bike path or infrastructure';
  if (f & FLAG_FACILITY) return 'Bike facility';
  return null;
}
function failReason(seg, rules) {
  const f = seg.flags || 0;
  const reasons = [];
  if ((f & FLAG_FREEWAY) && !rules.allowFreeways) reasons.push('limited-access freeway is not allowed');
  let shoulder = seg.sh;
...
if (!details || !details.summary || !Array.isArray(details.segs)) {
  ...
  report.innerHTML = '<div class="no-route">Plan a route on the map, then use “Route details & highlights” to see its road-by-road report.</div>';
} else {
  ...
  if (totals.failM > 0) {
    alert.hidden = false;
    alert.textContent = `${fmtDist(totals.failM)} of this route does not meet your riding rules. The exact sections are listed below.`;
  } else {
    alert.hidden = false;
    alert.classList.add('good');
    alert.textContent = 'This route stays within your current riding rules.';
  }
  ...
  const bike = sections(segs, (s) => !!bikeUse(s), (s) => ({
    name: roadName(s), meta: bikeUse(s),
  }));

  renderSection(report, 'Freeways', freeways, 'No freeway sections are used.', 'freeway');
  renderSection(report, 'Highways', highways, 'No highway sections are used.');
  renderSection(report, 'Does not meet your rules', failing, 'Every road section meets your current riding rules.', 'fail');
  renderSection(report, 'Bike routes & trails used', bike, 'No designated route, trail, or bike facility section is recorded on this route.');
}
